/**
 * قیمتو 5.2 — ICONS
 * ✅ فقط از فایل‌های موجود در assets/icons/ استفاده می‌کند
 * ✅ Fallback امن به SVG inline (بدون XSS)
 * ✅ پشتیبانی از نمادهای داینامیک TGJU
 */
window.Icons = (function(){
'use strict';

/* ============================================================
   UI ICONS — inline SVG (برای UI و fallback)
============================================================ */
const UI = {
  ...
  arrowDown: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M19 12l-7 7-7-7"/></svg>`,
...
/* ============================================================
   ✅ فقط فایل‌های موجود در assets/icons/ را map می‌کند
   بر اساس dir /s واقعی پروژه شما
============================================================ */
...
/* ============================================================
   ...
   فقط به فایل‌های موجود اشاره می‌کند
============================================================ */
const ASSET_ICON_MAP = {
  /* طلا و سکه → gold.png, coin.png */
  gold18: 'gold', gold24: 'gold', gold17: 'gold', gold17_coin: 'gold',
  gold17_transfer: 'gold', mesghal: 'gold', gold_melted: 'gold',
  gold_transfer: 'gold', gold_futures: 'gold', gold_mini: 'gold',
  gold_740k: 'gold', gold_world: 'gold', ounce: 'gold',
  coin: 'coin', coin_bahar: 'coin', nim: 'coin', rob: 'coin', gerami: 'coin',

  /* ارز → فایل‌های موجود */
  dollar: 'usd', dollar_dt: 'usd', dollar_ex: 'usd',
  euro: 'eur', gbp: 'gbp', pound: 'gbp',
  aed: 'aed', dirham: 'aed',
  try_: 'try', cny: 'cny', jpy: 'jpy',
...
/**
 * بررسی می‌کند که آیا فایل PNG برای این asset موجود است
 */
function hasIconFile(asset){
  if(!asset) return false;
  const fileName = ASSET_ICON_MAP[asset.id];
  return fileName && AVAILABLE_ICONS.has(fileName);
}

/**
 * نام فایل آیکون را برمی‌گرداند (یا null)
 */
...
/**
 * ✅ SVG امن برای asset — اگر PNG موجود بود، img، وگرنه SVG inline
 * ❌ بدون onerror attribute (جلوگیری از XSS)
 */
function assetSVG(asset){
  ...
  // Fallback به SVG inline بر اساس دسته‌بندی
  const catIconMap = {
    gold: 'coins',
    currency: 'dollarSign',
    metal: 'diamond',
  ...
  return `<span class="asset-icon-svg" data-icon-fallback="${iconName}">${get(iconName)}</span>`;
}

/**
 * Hydrate کردن [data-icon] elements
 */
...
/* ============================================================
   ✅ Global error handler برای img هایی که لود نمی‌شن
   (یک بار ثبت می‌شه، جایگزین onerror inline می‌شه)
============================================================ */
function installImageFallback(){
  ...
  document.addEventListener('error', e => {
    ...
    // اگه قبلاً fallback شده، دوباره تلاش نکن
